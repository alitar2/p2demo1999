        ...
        return http.csrf(c -> c.disable())
                .authorizeHttpRequests(auth ->
                        auth.requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()  // Allow all OPTIONS requests (lets the CORS preflight request pass)
                                .requestMatchers(new AntPathRequestMatcher("/users/**", HttpMethod.POST.toString())).permitAll() // Allow POST requests to /users
                                .requestMatchers("/auth/**").permitAll() // Allow all requests to /auth
                                .requestMatchers(new AntPathRequestMatcher("/users/**")).hasAuthority("manager") // Require manager authority for all other types of requests to /users
